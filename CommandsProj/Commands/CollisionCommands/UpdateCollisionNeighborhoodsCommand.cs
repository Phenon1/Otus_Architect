...
namespace CommandsProj.Commands.CollisionCommands
{
    public class UpdateCollisionNeighborhoodsCommand : ICommand
    {
            ...
            string macroPropertyName = CollisionNeighborhoodConstants.CollisionMacroPropertyName)
        {
            ...
            {
                throw new ArgumentException("Должна быть задана хотя бы одна система окрестностей.", nameof(neighborhoodSystems));
            }
            ...
            {
                throw new ArgumentException($"Идентификатор системы окрестностей '{duplicatedSystemId}' повторяется.", nameof(neighborhoodSystems));
            }
