...
namespace CommandsProj.Commands.CollisionCommands
{
    public class NeighborhoodSystem
    {
        ...
        public NeighborhoodSystem(string id, int cellWidth, int cellHeight, double offsetX = 0, double offsetY = 0)
        {
            ...
            {
                throw new ArgumentException("Neighborhood system id must not be empty.", nameof(id));
            }

            if (cellWidth <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(cellWidth), "Cell width must be positive.");
            }

            if (cellHeight <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(cellHeight), "Cell height must be positive.");
            }
